...
export function weekFor(residentId: string, addedEventId?: string): ScheduleEntry[] {
  ...
  const ids = already || !addedEventId
    ? existing
    : [...existing, addedEventId];

  return ids
    .map((id) => events.find((e) => e.id === id))
    .filter((e): e is SocialEvent => Boolean(e))
    .sort((a, b) => dayRank(a.startTime) - dayRank(b.startTime))
    .map((event) => ({
      event,
      isNew: event.id === addedEventId && !already,
      wasAlready: event.id === addedEventId && already,
    }));
}
